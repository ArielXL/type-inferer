...
class State:
    ...
    def graph(self):
        ...
        G.add_node(pydot.Node(START, shape=PLAIN_TEXT, label=NULL, width=0, height=0))
        ...
        def visit(start):
            ...
            if ids not in visited:
                visited.add(ids)
                G.add_node(pydot.Node(ids, label=start.name, shape=CIRCLE, style=BOLD if start.final else NULL))
        ...
        visit(self)
        G.add_edge(pydot.Edge(START, id(self), label=NULL, style=DASHED))
